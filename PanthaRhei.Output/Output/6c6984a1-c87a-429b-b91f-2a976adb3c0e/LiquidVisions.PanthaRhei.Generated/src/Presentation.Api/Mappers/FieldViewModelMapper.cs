...
namespace LiquidVisions.PanthaRhei.Generated.Presentation.Api.Mappers
{
    public class FieldViewModelMapper : IMapper<Field, FieldViewModel>
    {
        public void Map(Field source, FieldViewModel target)
        {
            ...
            target.Reference = new EntityModelMapper().Map(source.Reference);
            target.Entity = new EntityModelMapper().Map(source.Entity);
            target.IsKey = source.IsKey;
            target.IsIndex = source.IsIndex;
            target.RelationshipKeys = source.RelationshipKeys.Select(x => new RelationshipModelMapper().Map(x)).ToList();
            target.IsForeignEntityKeyOf = source.IsForeignEntityKeyOf.Select(x => new RelationshipModelMapper().Map(x)).ToList();
        }
