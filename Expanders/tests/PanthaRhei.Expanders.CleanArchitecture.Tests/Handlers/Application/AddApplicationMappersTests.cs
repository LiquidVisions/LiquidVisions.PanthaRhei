...
namespace LiquidVisions.PanthaRhei.Generator.CleanArchitecture.Tests.Handlers.Application
{
    public class AddApplicationMappersTests
    {
        ...
        public void Execute_ShouldRenderAnSaveApplicationMapperTemplate()
        {
            ...
            string expectedCreateFolder = Path.Combine(expectedCompontentOutputFolder, Resources.ApplicationMapperFolder, expectedEntity.Name.Pluralize());
            string[] expectedActions = new string[] { "Create", "Update" };
            fakes.IProjectAgentInteractor.Setup(x => x.GetComponentOutputFolder(fakes.ApplicationComponent.Object)).Returns(expectedCompontentOutputFolder);
